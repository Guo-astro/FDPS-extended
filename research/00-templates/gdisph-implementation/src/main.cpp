...
// Include the new output and unit system headers
#include "fdps_unit_system.hpp"
#include "fdps_output.hpp"
// Include GDISPH Riemann solver and force calculation
#include "riemann_solver.hpp"
#include "gdisph_force.hpp"
...
// Use GDISPH force calculation with Riemann solver
using CalcHydroForce = GDISPH::CalcGDISPHForceCase1;
...
inline std::tuple<PS::F64, PS::F64vec> GetConservativeVariables(const PS::ParticleSystem<FP>& sph_system){
   ...
   return std::make_tuple(Eng, Mom);
}

// Helper function to output using the new output system
template<typename UnitSystem>
void outputWithNewSystem(const PS::ParticleSystem<FP>& sph_system,
                        ...
                        PS::S32 my_rank)
{
    ...
    metadata.time = time;
    metadata.total_particles = sph_system.getNumberOfParticleGlobal();
    metadata.mpi_rank = PS::Comm::getRank();
    metadata.mpi_size = PS::Comm::getNumberOfProc();
    metadata.unit_system_name = getUnitSystemName<UnitSystem>();
    metadata.description = "SPH simulation data";

    // Create appropriate writer
    auto writer = createOutputWriter(format, base_filename, metadata, PS::CommInfo());

    if (writer && writer->open()) {
        // Write particle data
        const FP* particles = sph_system.getParticlePointer();
        PS::S64 num_local = sph_system.getNumberOfParticleLocal();
        
        writer->writeParticles<FP, UnitSystem>(particles, num_local);
        writer->close();

        if (my_rank == 0) {
            std::cout << "Output written: " << base_filename 
                     << getFileExtension(format) << std::endl;
        }
    } else {
        if (my_rank == 0) {
            std::cerr << "Failed to create output writer" << std::endl;
        }
    }
}
...
int main(int argc, char* argv[]){
   ...
   if (my_rank == 0) {
       std::cout << "===========================================" << std::endl
                 ...
                 << " with flexible unit and output systems!"     << std::endl
   ...
   auto [eng0, mom0] = GetConservativeVariables(sph_system);
   PS::S32 step = 0;
   for(PS::F64 time = 0 ; time < end_time ; time += dt, ++ step){
      ...
      // Output result files using the NEW flexible system
      if(step % OUTPUT_INTERVAL == 0){
         char filename[256];
         
         // Output in CSV format with Simulation units
         sprintf(filename, "result/snapshot_%04d_sim", step);
         outputWithNewSystem<PS::UnitSystem::SimulationUnit>(
             sph_system, PS::Output::OutputFormat::CSV, filename, time, my_rank);
         
         // Output in CSV format with CGS units
         sprintf(filename, "result/snapshot_%04d_cgs", step);
         outputWithNewSystem<PS::UnitSystem::CGSUnit>(
             sph_system, PS::Output::OutputFormat::CSV, filename, time, my_rank);
         
         // Output in CSV format with Galactic units
         sprintf(filename, "result/snapshot_%04d_galactic", step);
         outputWithNewSystem<PS::UnitSystem::GalacticUnit>(
             sph_system, PS::Output::OutputFormat::CSV, filename, time, my_rank);
         
#ifdef PARTICLE_SIMULATOR_USE_HDF5
         // Output in HDF5 format with Simulation units
         sprintf(filename, "result/snapshot_%04d_sim", step);
         outputWithNewSystem<PS::UnitSystem::SimulationUnit>(
             sph_system, PS::Output::OutputFormat::HDF5, filename, time, my_rank);
#endif
         
         // Also keep the original ASCII output for backwards compatibility
         FileHeader header;
         header.time = time;
         ...
         if (my_rank == 0){
            std::cout << "================================" << std::endl;
            std::cout << "output step " << step << " completed." << std::endl;
            std::cout << "================================" << std::endl;
         }
      }
      ...
      auto [eng1, mom1] = GetConservativeVariables(sph_system);
      std::cout<<"relative energy err: "<<(eng1 - eng0)/ eng0<<std::endl;
      std::cout<<"absolute momentum err: "<<(mom1 - mom0)<<std::endl;
   }
   // Finalize FDPS
   PS::Finalize();
   return 0;
