...
void makeColdUniformSphere(const PS::F64 mass_glb, const PS::S64 n_glb, const PS::S64 n_loc, PS::F64 *&mass, PS::F64vec *&pos, PS::F64vec *&vel,
...
int main(int argc, char *argv[]) {
	...
	std::cout<<"i= "<<i<<" acc= "<<system_grav[i].acc<<std::endl;
    }
    PS::Finalize();
    return 0;
    // CHECKED
    */
    ...
    tree_grav.calcForceAllAndWriteBackMultiWalk(DispatchKernelWithSP, RetrieveKernel, tag_max, system_grav, dinfo, n_walk_limit);
#elif defined(ENABLE_GPU_CUDA)
    const PS::S32 n_walk_limit = 4;
    const PS::S32 tag_max = 1;
    tree_grav.calcForceAllAndWriteBackMultiWalkIndex(DispatchKernelWithSPIndex, RetrieveKernel, tag_max, system_grav, dinfo, n_walk_limit);
#else
...
#if defined(MULTI_WALK_PTCL)
	// multi walk
        tree_grav.calcForceAllAndWriteBackMultiWalk(DispatchKernelWithSP, RetrieveKernel, tag_max, system_grav, dinfo, n_walk_limit, clear_force, list_mode);
#elif defined(ENABLE_GPU_CUDA)
	// multi walk + index
        tree_grav.calcForceAllAndWriteBackMultiWalkIndex(DispatchKernelWithSPIndex, RetrieveKernel, tag_max, system_grav, dinfo, n_walk_limit, clear_force, list_mode);
#else
        tree_grav.calcForceAllAndWriteBack(CalcGravity<FPGrav>, CalcGravity<PS::SPJMonopole>, system_grav, dinfo, clear_force, list_mode);
#endif
