...
inline __device__ float4 dev_gravity(float eps2, float3 ipos, float4 jposm, float4 accp) {
...
__device__ float4 ForceKernel_1walk(float4 *jpsh, const float3 ipos, const int id_walk, const int2 *ij_disp, const EpjGPU *epj, float4 accp,
                                    const float eps2) {
...
__device__ float4 ForceKernel_2walk(float4 jpsh[2][N_THREAD_GPU], const float3 ipos, const int id_walk, const int iwalk0, const int iwalk1,
                                    const int2 *ij_disp, const EpjGPU *epj, float4 accp, const float eps2) {
    ...
    for (int j = nj_shorter; j < nj_longer; j += N_THREAD_GPU) {
        ...
        if (jrem < N_THREAD_GPU) {
            for (int jj = 0; jj < jrem; jj++) {
                if (mywalk == walk_longer) accp = dev_gravity(eps2, ipos, jpsh[0][jj], accp);
            }
        } else {
#pragma unroll
            for (int jj = 0; jj < N_THREAD_GPU; jj++) {
...
__device__ float4 ForceKernel_multiwalk(const float3 ipos, const int id_walk, const int2 *ij_disp, const EpjGPU *epj, float4 accp, const float eps2) {
    const int j_head = ij_disp[id_walk].y;
    const int j_tail = ij_disp[id_walk + 1].y;
...
__global__ void ForceKernel(const int2 *ij_disp, const EpiGPU *epi, const EpjGPU *epj, ForceGPU *force, const float eps2) {
...
PS::S32 DispatchKernelWithSP(const PS::S32 tag, const PS::S32 n_walk, const FPGrav *epi[], const PS::S32 n_epi[], const FPGrav *epj[],
                             const PS::S32 n_epj[], const PS::SPJMonopole *spj[], const PS::S32 n_spj[]) {
...
PS::S32 RetrieveKernel(const PS::S32 tag, const PS::S32 n_walk, const PS::S32 ni[], FPGrav *force[]) {
    int ni_tot = 0;
    for (int k = 0; k < n_walk; k++) {
        ni_tot += ni[k];
    }
...
////////////////////
// INDEX VERSTION //
__device__ float4 ForceKernelIndex_1walk(float4 *jpsh, const float3 ipos, const int id_walk, const int2 *ij_disp, const EpjGPU *epj,
                                         const int *id_epj, float4 accp, const float eps2) {
    const int tid = threadIdx.x;
    const int j_head = ij_disp[id_walk].y;
    const int j_tail = ij_disp[id_walk + 1].y;
    for (int j = j_head; j < j_tail; j += N_THREAD_GPU) {
...
__device__ float4 ForceKernelIndex_2walk(float4 jpsh[2][N_THREAD_GPU], const float3 ipos, const int id_walk, const int iwalk0, const int iwalk1,
                                         const int2 *ij_disp, const EpjGPU *epj, const int *id_epj, float4 accp, const float eps2) {
    const int jbeg0 = ij_disp[iwalk0].y;
    const int jbeg1 = ij_disp[iwalk1].y;
    const int jend0 = ij_disp[iwalk0 + 1].y;
    const int jend1 = ij_disp[iwalk1 + 1].y;
...
__device__ float4 ForceKernelIndex_multiwalk(const float3 ipos, const int id_walk, const int2 *ij_disp, const EpjGPU *epj, const int *id_epj,
                                             float4 accp, const float eps2) {
    const int j_head = ij_disp[id_walk].y;
    const int j_tail = ij_disp[id_walk + 1].y;
    for (int j = j_head; j < j_tail; j++) {
...
__global__ void ForceKernelIndex(const int2 *ij_disp, const EpiGPU *epi, const EpjGPU *epj, const int *id_epj, ForceGPU *force, const float eps2) {
    int tid = blockDim.x * blockIdx.x + threadIdx.x;
    float3 ipos = epi[tid].pos;
    float4 accp = make_float4(0.f, 0.f, 0.f, 0.f);
    ...
    if (1 == nwalk_in_block) {
        ...
        int iwalk0 = epi[t_head].id_walk;
        int iwalk1 = epi[t_tail].id_walk;
        accp = ForceKernelIndex_2walk(jpsh, ipos, id_walk, iwalk0, iwalk1, ij_disp, epj, id_epj, accp, eps2);
    } else {
...
PS::S32 DispatchKernelWithSPIndex(const PS::S32 tag, const PS::S32 n_walk, const FPGrav **epi, const PS::S32 *n_epi, const PS::S32 **id_epj,
                                  const PS::S32 *n_epj, const PS::S32 **id_spj, const PS::S32 *n_spj, const FPGrav *epj, const PS::S32 n_epj_tot,
                                  const PS::SPJMonopole *spj, const PS::S32 n_spj_tot, const bool send_flag) {
    assert(n_walk <= N_WALK_LIMIT);
    if (init_call) {
        dev_epi.allocate(NI_LIMIT);
        dev_force.allocate(NI_LIMIT);
    ...
    if (send_flag == true) {
        if (NJ_LIMIT < n_epj_tot + n_spj_tot) {
            std::cerr << "n_epj_tot+n_spj_tot= " << n_epj_tot + n_spj_tot << " NJ_LIMIT= " << NJ_LIMIT << std::endl;
        }
        assert(NJ_LIMIT > n_epj_tot + n_spj_tot);
        // const auto wtime0 = MPI_Wtime();
        PS_OMP_PARALLEL {
            PS_OMP_FOR
    ...
    ij_disp.htod(n_walk + 2);  // ij_disp[n_walk+1] equals to ij_disp[n_walk]
    int ni_tot_reg = ij_disp[n_walk].x;
    if (ni_tot_reg % N_THREAD_GPU) {
        ni_tot_reg /= N_THREAD_GPU;
        ni_tot_reg++;
    ...
    // const PS::F64 wtime0 = MPI_Wtime();
    // PS::F64 wtime1, wtime2;
...
#if 0
__device__ float3 Kick(const float3 vel, const float4 accp, const double dt) {
    float3 vel_new;
...
__device__ float3 Drift(const float3 pos, const float3 vel, const double dt) {
    float3 pos_new;
...
// optimized version
__global__ void ForceKernelKickDrifgIndex(const int2 *ij_disp, const EpiGPU *epi, const EpjGPU *epj, const int *id_epj, ForceGPU *force,
                                          const float eps2) {
    int tid = blockDim.x * blockIdx.x + threadIdx.x;
    float3 ipos = epi[tid].pos;
    int id_walk = epi[tid].id_walk;
    ...
    float3 vel = epi[tid].vel;
    force[tid].vel = Kick(vel, accp, DELTA_TIME);
    force[tid].pos = Drift(ipos, force[tid].vel, DELTA_TIME);
    force[tid].accp = accp;
}
#endif